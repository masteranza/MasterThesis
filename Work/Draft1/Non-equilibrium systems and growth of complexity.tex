...
and it is easy to verify that the entropy of a pure state is equal to zero. The entropy of non-pure state is always greater than zero.
...
In second quantization formalism the measurement of a particle is realized by Field operator $ \hat{\Psi}(x) =\sum_k \phi_k(x)\hat{a}_n$ which annihilates a single particle at position $x$.
...
Now depending on the precision of the measurement we can recalculate entropy of course getting a value greater then zero. If we would perform the same analysis for a pure state of two particles in different states i.e. $ket{1,1,...}$ then we would obtain an increase of entropy even without accounting for the lost particle.
%% Correct also for correlated systems?
...
%Jaynes - Gibbs vs Boltzmann Entropies
...
%Practical modification on pages 55-56
...
%Connection to f-Liouville operator if needed page 66.
...
getting the Dissipation Theorem which states that the nonlinear response of an arbitrary phase variable can be calculated from the time integral of the non-equilibrium transient time correlation function of the phase variable with the dissipation function.
...
and is called transient time correlation function (TTCF).
%More on usage page 71
