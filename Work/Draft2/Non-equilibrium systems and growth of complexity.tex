...
\subsection{Green-Kubo relations [in progress]}
...
The interpretation of the term in the exponent, can be done by noticing that there's only one known macroscopic quantity\footnote{Up to a numerical factor - $k_B^{-1}$} that is strictly positive and can change signs after reversing the left side, namely the standard measure of irreversibility - entropy.

We have thus, described entropy produced by a macroscopic system solely in terms of microscopic transition probabilities between two macroscopic states, without any assumptions about the dynamics and external forces influencing the system.
...
The remarkable feature of this results is that the obtained distributions are not gaussian! As we will see shortly, this might be seen %MOST LIKELY
as the result of correspondence with the so-called Fluctuation Theorems.
%TODO: Is it possible to modify this to fluctuation theorem????

\section{Fluctuation theorems}

In the previous chapter we have seen how irreversibility arrises in macroscopic systems, a complementary approach can be described using rather undemanding assumptions.
This chapter describes the fluctuation theorems, that emerge from the analysis of a physical system interacting with its environment and rest upon the same, natural measure of irreversibility introduced before.
