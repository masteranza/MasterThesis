...
Linear response of a system driven from equilibrium can be described in terms of the \textit{time correlation function} of the observable $X$\footnote{From now on we will assume that $X$ is mean zero observable, that is $X=\delta X$}:
...
On the microscopic level of enumerated, time dependent observables $X_i$, the equations of motion are time reversible and time translation invariant, thus leading to
...
In 1931, Onsager shown \cite{Anonymous:zgWBDrcO} that $\gamma_{ik}$ from the previous paragraph is in fact symmetric.
...
However, Onsager conjectured that in the linear approximation they should be equal. To see why this is the case we give a heuristic argument.
Let's consider the expression for $\bar{X}$ for time $t < 0$:
...
As a practical note on application of Onsager relations, we quote Charles Kittel:
