...
In case of pure non-equilibrium fewer results are known in general. However some remarkable progress has been made in recent years.




\subsection{A word from equilibrium statistical mechanics}

In the following paragraph we show that Fluctuation Theorem is in fact, not a novel result. One can in fact obtain not only it, but also the exact form of the entropy produced in terms of microscopic transition probabilites, using nothing else than simple probability calculus and the hypothesis of equal apriori probabilities.
...
The interpretation of the term in the exponent, can be done by noticing that there's only one known macroscopic quantity\footnote{Up to a numerical factor - $k_B^{-1}$} that could change signs after reversing the left side, the measure of irreversibility - entropy. We have thus, described entropy produced purely in terms of microscopic transition probabilities between two macroscopic states, with no assumption about the dynamics nor about external forces influencing the system.

With the use random matrices, satisfying the conditions $\sum_j K_{kj} = 1$ for any $k$, to obtain the distributions for the possible values of entropy produced in the transition $ S = \ln{\sum_{k,m} \frac{K_{kj}}{K_{km}}} $. The results, for $N=10000$ random matrices are presented on figure \ref{Fig4}.

\begin{figure}[ht!]
\centering \includegraphics[width=8cm]{dissipation} \caption{Distribution of generic dissipation function for $N_A = 2$ and $N_B=2$}
\label{Fig4} 
\end{figure}
...
Fewer technical difficulties, by assuming stochastic dynamics \cite{Crooks:2008ta}. 
