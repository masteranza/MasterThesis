...
\begin{equation}
\langle \langle e^{- \beta \Delta Q_{i\to j}^{\tau}} \rangle_{i \to j} e^{\ln[\frac{p(j|II)}{p(i|I)}]} e^{-\ln[\frac{\pi(II \to I)}{\pi(I \to II)}]} \rangle_{I \to II}
\end{equation}
and by making use of $e^x \geq 1+x$ one gets\footnote{One can safely extend the trajectory $\langle ... \rangle_{i \to j}$ average to all three terms}

\begin{equation}
  \beta \langle\langle \Delta Q_{i\to j}^{\tau} \rangle_{i \to j} \rangle_{I \to II}+\langle \ln[\frac{p(i|I)}{p(j|II)}]  \rangle_{I \to II}+ \ln{\frac{\pi(II \to I)}{\pi(I \to II)}} \geq 0.
\end{equation}
...
\begin{equation}
\label{SecondLawII}
  \beta \langle\langle \Delta Q_{i\to j}^{\tau} \rangle_{i \to j} \rangle_{I \to II} + \ln{\frac{\pi(II \to I)}{\pi(I \to II)}} +\Delta S_{int}  \geq 0.
\end{equation}
...
The obtained result can be applied to a simple model of self-replicators. Lets suppose we have a master equation for $n \gg 1$ governing the population
...
If we now connect the state of living with macrostate II and the state dead state with macrostate I, then naturally we assign $\pi (I \to II) = g\ dt$ and $\pi (II \to I) = \delta\ dt$ for some time $dt$.

The equation then (\ref{SecondLawII}) dictates

\begin{equation}
  \Delta S_{int} + \beta \langle\langle \Delta Q_{i\to j}^{\tau} \rangle_{i \to j} \rangle_{I \to II} \geq \ln{\frac{g}{\delta}}.
\end{equation}
...
\begin{equation}
  g \leq g_{max}=\delta \exp[ \Delta S_{int} + \beta \langle\langle \Delta Q_{i\to j}^{\tau} \rangle_{i \to j} \rangle_{I \to II}].
\end{equation}

Most general observation that can be made from this equation is that  in order for the growth rate $g$ to exceed the die rate $\delta $ the negative internal entropy change must be paid by the (strictly larger) dissipated heat.
...
We have now seen how recent progress in microscale non-equilibrium statistical physics has allowed us to make successful predictions  about inanimate matter. Indeed the fruitfulness of this approach were noticed by other groups who seek to further generalize it - this will be the topic of next paragraph.
...
\subsection{MEP principle as an inference principle}
